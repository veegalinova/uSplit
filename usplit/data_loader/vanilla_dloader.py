...
class MultiChDloader:
    ...
    def __getitem__(self, index: Union[int, Tuple[int, int]]) -> Tuple[np.ndarray, np.ndarray]:
        img_tuples = self._get_img(index)  # 2 elems
        inp, target = img_tuples
        ...
        # target should be just the channel

        # do NOT normalize target, input normalization in compute input
